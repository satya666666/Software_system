...
int main() {
    ...
    if ((pid = fork()) == -1)                  // Fork the first child process for 'ls -l'
    {
        perror("fork");
        exit(EXIT_FAILURE);
    }
    if (pid == 0) {                             // Child process for 'ls -l'
        ...
        dup(pipefd[1]);                         // Duplicate write end to lowest unused fd (STDOUT)
    ...
    if (pid == 0) {                              // Child process for 'wc'
        dup(pipefd[0]);                          // Duplicate read end to lowest unused fd (STDIN)
...
// satyam@satyam-X542UQ:~/Desktop/ss_list2$ cc 17a.c
...
// -rw-rw-r-- 1 satyam satyam  1823 Sep 18 23:12 17a.c
// -rw-rw-r-- 1 satyam satyam   731 Sep 18 22:57 17b.c
// -rw-rw-r-- 1 satyam satyam   599 Sep 18 22:55 17c.c
...
// -rwxrwxr-x 1 satyam satyam 16304 Sep 18 23:12 a.out
